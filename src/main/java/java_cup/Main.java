 ...
 *   <dt> -parser_ name
 *   <dd> specify parser_ class name [default "parser_"]
 *   <dt> -symbols name
...
public class Main {
  ...
  /** User option -- do we assembleAll produce extra debugging messages */
  ...
  public static void main(String argv[])
      ...
      if (lexer.error_count == 0)
	{
	  ...
	  /* output the generated code */
          if (print_progress) System.err.println("Writing parser_...");
          emit_parser();
	  did_output = true;
...
"    -parser_ name   specify parser_ class name [default \"parser_\"]\n" +
...
"    -dump          produce a dump of all of the above\n"
  ...
  protected static void parse_args(String argv[])
      ...
      for (i=0; i<len; i++)
	{
	  ...
	  else if (argv[i].equals("-parser_"))
	    {
	      /* must have an arg */
	      if (++i >= len || argv[i].startsWith("-") ||
				argv[i].endsWith(".cup"))
		usage("-parser_ must have a name argument");
  ...
  /** Output file for the parser_ class. */
  ...
  protected static void parse_grammar_spec() throws java.lang.Exception
    {
      ...
      /* create a parser_ and parse with it */
  ...
  /** Build the (internal) parser_ from the previously parsed specification.
   *  This includes:<ul>
  ...
  protected static void build_parser() throws internal_error
	...
	System.err.println("  Building state machine...");
      ...
      /* build the LR parser_ action and reduce-goto tables */
      if (opt_do_debug || print_progress)
  ...
  /** Call the emit routines necessary to write out the generated parser_. */
