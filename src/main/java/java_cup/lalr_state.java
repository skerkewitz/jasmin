 ...
 *  a potential configuration of the parser_.  If the item set of a state
 *  includes an item such as: <pre>
 *    [A ::= B * C d E , {a,b,c}]
 *  </pre> 
 *  this indicates that when the parser_ is in this state it is currently
 ...
 *  the parser_ is normally looking for several things at once (represented
 ...
 *
 *  At runtime, the parser_ uses a viable prefix recognition machine made up
 *  of these states to parse.  The parser_ has two operations, shift and reduce.
 ...
 *  the end of the transition).  For a reduce operation, the parser_ is
 ...
 *  parser_ first attempted to find the RHS.  From this state it makes a
...
public class lalr_state {
   ...
   *  the parser_ was now looking for.  In this case we would items
