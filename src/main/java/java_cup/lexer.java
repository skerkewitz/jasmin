 ...
 *    "parser_"      PARSER                "terminal"    TERMINAL
 *    "non"         NON                   "init"        INIT 
 *    "scan"        SCAN                  "with"        WITH
 *    "start"       START                   ;           SEMI 
 *      ,           COMMA                   *           STAR 
...
public class lexer {
  ...
  public static void init() throws java.io.IOException
    {
      ...
      keywords.put("parser_",   sym.PARSER);
      ...
      keywords.put("debug",    sym.DEBUG);

      /* set up the table of single character symbols */
      char_symbols.put(';', sym.SEMI);
      char_symbols.put(',', sym.COMMA);
      char_symbols.put('*', sym.STAR);
      char_symbols.put('.', sym.DOT);
      char_symbols.put('|', sym.BAR);
  ...
  protected static token do_code_string() throws java.io.IOException
	...
	{
	  /* if we have assembleAll off the end issue a message and break out of loop */
	  if (next_char == EOF_CHAR)
	    {
   ...
   *  parser_, this can be changed to call debug_next_token() which prints
