...
/**
 * This class implements a skeleton table driven LR parser_.  In general,
 ...
 * the parser_ uses a state machine (the "viable prefix recognition machine"
 * built by the parser_ generator).  The current state of the machine is placed
 ...
 * determine whether to shift or to reduce.  When the parser_ shifts, it
 * changes to a new state by pushing a new symbol (containing a new state)
 * onto the stack.  When the parser_ reduces, it pops the handle (right hand
 * side of a production) off the stack.  This leaves the parser_ in the state
 ...
 * indexes) to determine a new state to go to.  The parser_ then shifts to
 ...
 * debug_parse() provide two versions of the main parser_ (the only difference
 ...
 * input in order to execute all actions and re-synchronize the actual parser_
 * configuration.<p>
 * <p>
 * This is an abstract class which is normally filled out by a subclass
 * generated by the JavaCup parser_ generator.  In addition to supplying
...
public abstract class lr_parser {
   ...
   * be taken when the parser_ is in the given state (i.e., the given state
  ...
  /**
   * Internal flag to indicate when parser_ should quit.
   */
  ...
  /**
   * This method is called to indicate that the parser_ should quit.  This is
   ...
   * at parser_ generation time.
   *
   * @param act_num the internal index of the action to be performed.
   * @param parser  the parser_ object we are acting for.
   * @param stack   the parse stack of that object.
   * @param top     the index of the top element of the parse stack.
   */
  public abstract symbol do_action(int act_num, lr_parser parser, Stack stack,int top) throws jasError, IOException, internal_error;

  /*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/

  /**
   * User code for initialization inside the parser_.  Typically this
   * initializes the scanner.  This is called before the parser_ requests
   ...
   * clause as its body.
   */
  public void user_init() throws IOException {
  }

  /*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/

  /**
   * Initialize the action object.  This is called before the parser_ does
   ...
   * "scan with" clause.
   */
  public abstract token scan() throws IOException, jasError;
  ...
  public void report_fatal_error(String message, Object info) {
    ...
    /* throw an exception */
    throw new RuntimeException("Can't recover from previous error(s)");
  }
  ...
  protected final short get_action(int state, int sym) {
    ...
    if (row.length < 20)
      ...
      /* shouldn't happened, but if we assembleAll off the end we return the
	 default (error == 0) */
    return 0;
  }
  ...
  protected final short get_reduce(int state, int sym) {
    ...
    /* if we assembleAll off the end we return the default (error == -1) */
    return -1;
  }
   ...
   * done_parsing() has been called (typically because the parser_ has
   * accepted, or a fatal error has been reported).  See the header
   * documentation for the class regarding how shift/reduce parsers operate
   * and how the various tables are used.
   */
  public void parse() throws IOException, jasError, internal_error {
    ...
    /* set up direct reference to tables to drive the parser_ */
   ...
   * debug_reduce() when shift and reduce moves are taken by the parser_
   * and produces various other debugging messages.
   */
  public void debug_parse() throws IOException, jasError, internal_error {
    /* the current action code */
    int act;
    ...
    debug_message("# Initializing parser_");
   ...
   * parser_'s configuration, nor execute any actions. If we can  parse all
   ...
   * normal parser_ to continue with the overall parse.
   *
   * @param debug should we produce debugging messages as we parse.
   */
  protected boolean error_recovery(boolean debug) throws IOException, jasError, internal_error {
   ...
   * symbols.
   */
  protected void read_lookahead() throws IOException, jasError {
   ...
   */
  protected void restart_lookahead() throws IOException, jasError {
  ...
  protected boolean try_parse_ahead(boolean debug) {
    ...
    for (; ; ) {
      ...
      if (act > 0) {
        ...
        /* advance simulated input, if we assembleAll off the end, we are done */
   ...
   * control back to the normal parser_.  Consequently, this version of the
   * parser_ performs all actions and modifies the real parse configuration.
   * This returns once we have consumed all the stored input or we accept.
   *
   * @param debug should we produce debugging messages as we parse.
   */
  protected void parse_lookahead(boolean debug) throws IOException, jasError, internal_error {
    /* the current action code */
    int act;
